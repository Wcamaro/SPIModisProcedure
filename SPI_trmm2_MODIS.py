...
from process_trmm_3b42_daily import init,  WriteGTiff_2, cumulatedict, media, medialn, probnorain, SPI_DICT, calendardays
import datetime
...
for cumulated in cumulateds: # Number of months to cumulate; i.e: 1 Monthly_Spi; 3 Tri_Monthly_SPI; 6 Semestral_SPI...
    ...
    for dat in dates:
        ...
        if dict_matrix == {}:
            ...
            folder_output = r'D:\Python_Projects\Results\TRMM\SPI_MODIS_2017\%s' % (cumulated)
            if not os.path.exists(folder_output):
                os.makedirs(folder_output)
            WriteGTiff_2(dict_SPI,folder_output,'SPI_%smonthly.%s' % (cumulated,str(dat)),bbox['left'],bbox['right'],bbox['bottom'],bbox['top'],dat)
